...
void ParticleSystemSpawnTexture::init( int _texSize )
{
	...
	guiLightAndMaterial.setPosition( 600,10 );
	...
	ofPrimitiveMode primitiveMode = OF_PRIMITIVE_POINTS; // as we'll be drawing ths mesh instanced many times, we need to have many triangles as opposed to one long triangle strip
	ofMesh tmpMesh;
...
//-----------------------------------------------------------------------------------------
//
void ParticleSystemSpawnTexture::draw(  )
{
	ofEnableLighting();

		drawParticles( &particleDraw );
...
//-----------------------------------------------------------------------------------------
//
void ParticleSystemSpawnTexture::drawParticles( ofShader* _shader )
{
	...
	_shader->begin();
		...
		particleMaterial.begin();
			...
			singleParticleMesh.drawInstanced( OF_MESH_POINTS, numParticles );
